...
impl Options {
    ...
    fn start<E>(&self, started_at: Instant, deadline: Option<Instant>) -> EaseOff<E> {
        EaseOff {
            core: EaseOffCore::new(self.clone(), deadline),
            started_at,
            last_error: None,
        }
    }
...
impl<E> EaseOff<E> {
    ...
    fn next_sleep_until(&mut self) -> Result<Option<Instant>, Error<E>> {
        ...
        if self.last_error.is_none() {
            return Ok(self.core.initial_sleep_until());
        }

        match self.core.next_sleep_until() {
            Ok(sleep_until) => Ok(Some(sleep_until)),
            Err(_) => Err(Error::TimedOut(TimeoutError {
                last_error: self
                    .last_error
                    .take()
                    .expect("BUG: `last_error.is_none()` was checked above"),
            })),
        }
    }
}
